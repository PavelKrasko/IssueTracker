...
export default function AddTaskModal() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (!userId) return;
            const resProj = await fetch(`/api/tasks/projects/${userId}`);
            if (resProj.ok) {
                setProjects(await resProj.json());
            }

            const resDefects = await fetch('/api/tasks/defect-types');
            if (resDefects.ok) setDefectTypes(await resDefects.json());

            const resRoles = await fetch('/api/tasks/roles');
                ...
                <form className="admin-project-form" onSubmit={handleSubmit}>
                    ...
                    <div className="admin-input-wrapper">
                        ...
                        <select name="status" className="admin-field-input" required defaultValue="">
                            ...
                            <option value="Открыта">Открыто</option>
                        </select>
                    </div>
